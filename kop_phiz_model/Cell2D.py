import numpy as np # Импорт библиотеки для работы с массивами и обозначение её как np
import matplotlib.pyplot as plt # Импорт библиотеки для создания графиков и обозначение её как plt
from time import sleep # Импорт функции sleep из библиотеки time для управления задержками
from IPython.display import clear_output # Импорт функции clear_output из IPython.display для очистки вывода

from utils import underride # Импорт функции underride из модуля utils
...
def draw_array(array, **options):
    ...
    options = underride(options,
                        cmap='Greens', # Цветовая карта для отображения (зеленый)
                        ...
                        extent=[0, m, 0, n]) # Диапазон значений по осям X и Y

    plt.axis([0, m, 0, n]) # Установка пределов осей
    plt.xticks([]) # Удаление делений по осям X
